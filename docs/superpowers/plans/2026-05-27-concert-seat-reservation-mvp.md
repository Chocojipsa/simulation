...
**Architecture:** Use one Spring Boot backend codebase with API and worker profiles, two API containers behind Nginx, one worker container, Redis, PostgreSQL, Kafka, and a thin Next.js frontend. The synchronous path handles queue entry, admission, seat holds, and durable reservation state; the asynchronous path handles payment simulation and dashboard events.

**Tech Stack:** Java 21, Spring Boot, Spring MVC, Spring Data JPA, Spring Data Redis, Spring Kafka, PostgreSQL, Redis, Kafka, Docker Compose, Next.js, TypeScript, SSE, JUnit, Testcontainers.
...
- `backend/build.gradle.kts`: backend Gradle build configuration.
...
Expected: `gradlew`, `gradlew.bat`, and `gradle/wrapper/*` are created. If local Gradle is not installed, install Gradle or create the wrapper through an IDE before continuing.
...
## Task 8: Local Infrastructure With Two API Containers
...
git commit -m "chore: add local multi-server infrastructure"
```
